...
- **Costing Viewer** — the same access and limitations as Viewer, plus permission to edit the Hours Log and manage Costing Buttons in Settings. A pencil action unlocks job number and timestamps for regular entries; non-job costing activities keep their activity name fixed and allow timestamp corrections only. Corrections record the editor and timestamp, deleting an entry requires confirmation, and the log fits without horizontal scrolling by using a compact table on desktop and entry cards on smaller screens. Every role with Hours Log access picks the range from the same day bar the Job Map uses — arrows step a day at a time, and the label opens a calendar where one tap starts a range and a second closes it (the same day twice means a single day) — then exports the visible range as an Excel workbook
